		...
		<div>
			<Drawer docked={isDocked} open={isOpen} containerClassName="admin_drawer_class" onRequestChange={handleRequestChange}>
				<Link to="/" style={{ textDecoration: "none" }}>
		          	<MenuItem 
		         		className={selected === 1 ? "admin_menu_class add_background" : "admin_menu_class"} 
		         		onClick={() => handleNavClick(1)}
		         		primaryText="Retrieve"
		           	>
			          	<img 
			          		src={selected === 1 ? whiteCreateIcon : createIcon}
	          	...
	          	<Link to="/create" style={{ textDecoration: "none" }}>
		          	<MenuItem 
		          		className={selected === 2 ? "admin_menu_class add_background" : "admin_menu_class"} 
		          		onClick={() => handleNavClick(2)}
		          		primaryText="Create"
		          	>
			          	<img 
			          		src={selected === 2 ? whiteRetrieveIcon : retrieveIcon}
			          		className="admin_img_icon_size"
			          		alt="Create"
			          	/>
		          	</MenuItem>
	          	...
	          	<Link to="/update" style={{ textDecoration: "none" }}>
	          		<MenuItem 
	          			className={selected === 3 ? "admin_menu_class add_background" : "admin_menu_class"} 
	          			onClick={() => handleNavClick(3)}
	          			primaryText="Update"
	          		>
		          		<img 
		          			src={selected === 3 ? whiteUpdateIcon : updateIcon}
		          			className="admin_img_icon_size"
		          			alt="Update"
		          		/>
	          		</MenuItem>
	          	...
	          	<Link to="/delete" style={{ textDecoration: "none" }}>
	          		<MenuItem 
	          			className={selected === 4 ? "admin_menu_class add_background" : "admin_menu_class"} 
	          			onClick={() => handleNavClick(4)}
	          			primaryText="Delete"
	          		>
		          		<img 
		          			src={selected === 4 ? whiteTrashIcon : trashIcon}
		          			className="admin_img_icon_size"
		          			alt="Delete"
		          		/>
	          		</MenuItem>
