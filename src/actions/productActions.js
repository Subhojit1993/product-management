...
// product url set
const productUrl = `https://db-collection-subhojit-debnath-93.onrender.com/collections/products/`;
...
export function searchProducts(param) {
  return (dispatch) => {
    dispatch(fetchProductsBegin());
    return axios
      .get(`${productUrl}?quer=${param}`)
      .then((json) => {
        dispatch(searchProductsSuccess(json.data));
        return json.data;
      })
...
export function createProducts(dataAction) {
  return (dispatch) => {
    dispatch(fetchProductsBegin());
    return axios
      .post(`${productUrl}`, dataAction.data)
      .then((resp) => {
...
export function updateProducts(dataAction) {
  return (dispatch) => {
    dispatch(fetchProductsBegin());
    return axios
      .patch(`${productUrl}${dataAction.setId}`, dataAction.product)
      .then((resp) => {
